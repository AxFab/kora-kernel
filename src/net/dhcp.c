...
int dhcp_packet(netdev_t *ifnet, const uint8_t *ip, uint32_t uid, int mode, const dhcp_info_t *info, const dhcp_lease_t *lease);
...
static void dhcp_write_options_request(netdev_t *ifnet, skb_t *skb, uint8_t option[32], const uint8_t *ip, const dhcp_info_t *info)
{
...
static int dhcp_count_options_reply(netdev_t *ifnet, const uint8_t *ip, const dhcp_info_t *info, const dhcp_lease_t *lease)
{
	int opts = 3 * 6;
	if (lease != NULL)
	    opts += 6;
...
static void dhcp_write_options_reply(netdev_t *ifnet, skb_t *skb, uint8_t option[32], const uint8_t *ip, const dhcp_info_t *info, const dhcp_lease_t *lease)
{
	option[0] = DHCP_OPT_SERVERIP;
	option[1] = IP4_ALEN;
    memcpy(&option[2], ifnet->ip4_addr, IP4_ALEN);
...
int dhcp_packet(netdev_t *ifnet, const uint8_t *ip, uint32_t uid, int mode, const dhcp_info_t *info, const dhcp_lease_t *lease)
{
    ...
    option[1] = ETH_ALEN + 1;
    option[2] = 1;
    memcpy(&option[3], bt_mode == BOOT_REQUEST ? ifnet->eth_addr: skb->eth_addr, ETH_ALEN);
    net_write(skb, option, option[1] + 2);
...
int dhcp_discovery(netdev_t *ifnet)
{
	...
	splock_unlock(&ifnet->lock);
	return dhcp_packet(ifnet, ip4_broadcast, ifnet->dhcp_transaction, DHCP_DISCOVER, NULL, NULL);
}

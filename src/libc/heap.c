...
static heap_arena_t *new_arena(size_t length)
{
  ...
  setup_arena(arena, (size_t)map, length, __arena_chunk_size_limit, __arena_option);
...
void setup_allocator(void* base, size_t len)
{
  ...
  setup_arena(&__firstArena, (size_t)base, len, __arena_chunk_size_limit, __arena_option);

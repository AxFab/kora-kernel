...
void dev_setup()
{
    ...
    // null_ino->dev->block = 1;
    // null_ino->dev->ops = &dev_ops;
    // null_ino->ops = &null_ino_ops;
    // null_ino->dev->devclass = (char *)"Bytes device";
    // vfs_mkdev(null_ino, "null");
    // vfs_close(null_ino);

    // inode_t *zero_ino = vfs_inode(2, FL_CHR, NULL);
    // zero_ino->dev->block = 1;
    // zero_ino->dev->flags = VFS_RDONLY;
    // zero_ino->dev->ops = &dev_ops;
    // zero_ino->ops = &zero_ino_ops;
    // zero_ino->dev->devclass = (char *)"Bytes device";
    // vfs_mkdev(zero_ino, "zero");
    // vfs_close(zero_ino);

    // inode_t *rand_ino = vfs_inode(3, FL_CHR, NULL);
    // rand_ino->dev->block = 1;
    // rand_ino->dev->flags = VFS_RDONLY;
    // rand_ino->dev->ops = &dev_ops;
    // rand_ino->ops = &rand_ino_ops;
    // rand_ino->dev->devclass = (char *)"Bytes device";

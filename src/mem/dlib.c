...
static inode_t *dlib_lookfor_at(vfs_t *vfs, acl_t *acl, const char *name, const char *xpath)
{
    char *rl;
    ...
    for (char *dir = strtok_r(path, ":;", &rl); dir; dir = strtok_r(NULL, ":;", &rl)) {
        ...
        inode_t *ino = vfs_search_ino(vfs, buf, acl, true);
        if (ino != NULL) {
            kfree(buf);
            kfree(path);
            return ino;
...
static inode_t *dlib_lookfor(vfs_t *vfs, acl_t *acl, const char *name, const char *lpath, const char *rpath, const char *sys, bool req_uid)
{
    inode_t *ino = vfs_search_ino(vfs, name, acl, true);
    if (ino != NULL || strchr(name, '/'))
        return ino;

    if (rpath != NULL) {
        ino = dlib_lookfor_at(vfs, acl, name, rpath);
        if (ino != NULL)
            return ino;
    }
    ...
    if (!req_uid) {
        if (lpath != NULL) {
            ino = dlib_lookfor_at(vfs, acl, name, lpath);
            if (ino != NULL)
                return ino;
        }
    }

    return  dlib_lookfor_at(vfs, acl, name, sys);
}
...
int dlib_open(dlproc_t *proc, mspace_t *mm, vfs_t *vfs, acl_t *acl, const char *name)
{
    ...
    inode_t *ino = dlib_lookfor(vfs, acl, name, path, NULL, "/usr/bin:/bin", false);
    ...
    while (queue.count_ > 0) {
        ...
        for ll_each (&lib->depends, dep, dlname_t, node) {
            ...
            if (sl == NULL) {
                ...
                ino = dlib_lookfor(vfs, acl, dep->name, lpath, proc->exec->rpath, "/usr/lib:/lib", req_uid);

...
char *vma_print(char *buf, int len, vma_t *vma)
{
    ...
    char *nam;
    int i = 0;
    char sh = vma->flags & VMA_COPY_ON_WRITE ? (vma->flags & VMA_SHARED ? 'W' : 'w') : (vma->flags & VMA_SHARED ? 'S' : 'p');
...
int vma_resolve(vma_t *vma, size_t address, size_t length)
{
    ...
    case VMA_PHYS:
        while (length > 0) {
            mmu_resolve(vma->mspace, address, (page_t)offset, vma->flags & VMA_RIGHTS, false);
            length -= PAGE_SIZE;
            address += PAGE_SIZE;
            offset += PAGE_SIZE;
        }
    ...
    case VMA_ANON:
        while (length > 0) {
            mmu_resolve(vma->mspace, address, 0, vma->flags & VMA_RIGHTS, true);
            length -= PAGE_SIZE;
            address += PAGE_SIZE;
        }
        break;
    case VMA_FILE:
        while (length > 0) {
            ...
            mmu_resolve(vma->mspace, address, pg, vma->flags & VMA_RIGHTS, false);
            length -= PAGE_SIZE;
            address += PAGE_SIZE;
            offset += PAGE_SIZE;
        }

...
static void vfs_destroy_fsnode(device_t *dev, fnode_t *node, char *key)
{
    ...
    if (node->parent == NULL) {
        kprintf(KL_FSA, "Release root fsnode `%s`\n", node->name);
...
void vfs_fnode_zombify(fnode_t *node)
{
    ...
    mtx_lock(&parent->mtx);
    ll_remove(&parent->mnt, &node->nmt);
    // TODO -- Remove node as child of parent !!!! (ZOMBIE FNODE)!!
    vfs_close_fsnode(node);
    // node->parent = NULL;
    // Remov from parent device ?
    mtx_unlock(&parent->mtx);
...
static inline int vfs_open_access(int options)
{
    int access = 0;
    if (options & O_RDONLY)
        access = VM_RD;
...
inode_t *vfs_mkdir(vfs_t *vfs, const char *name, acl_t *acl, int mode)
{
    ...
    assert(irq_ready());
    inode_t *ino = dir->ops->mkdir(dir, node->name, mode & (~vfs->umask & 07777), acl);
    if (ino != NULL)
...
inode_t *vfs_open(vfs_t *vfs, const char *name, acl_t *acl, int mode, int flags) 
{
    ...
    vfs_close_fsnode(node);
    if (!created && vfs_access(ino, acl, access) != 0) {
        vfs_close_inode(ino);
        errno = EACCES;
        return NULL;
    }
    ...
    if (flags & O_TRUNC) {
        if (!created && vfs_access(ino, acl, VM_RW) != 0) {
            vfs_close_inode(ino);
            errno = EPERM;
            return NULL;
        }
...
inode_t *vfs_symlink(vfs_t *vfs, const char *name, acl_t *acl, const char *path) 
{
    ...
    inode_t *ino = dir->ops->symlink(dir, node->name, acl, path);
    if (ino != NULL)
        vfs_resolve(node, ino);
    mtx_unlock(&node->mtx);
    return ino;

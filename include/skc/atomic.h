...
__stinline void cpu_enable_irq()
{
#if defined USE_IRQ
  asm volatile ("sti");
#endif
...
__stinline void cpu_disable_irq()
{
#if defined USE_IRQ
  asm volatile ("cli");
#endif
...
uint16_t atomic16_xadd(atomic16_t *ptr, uint16_t value);
uint16_t atomic16_cmpxchg(atomic16_t *ptr, uint16_t reference, uint16_t value);
...
uint32_t atomic32_cmpxchg(atomic32_t *ptr, uint32_t reference, uint32_t value);

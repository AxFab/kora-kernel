...
const char *ksymbol(size_t ptr, char *buf, size_t len)
{
    buf[0] = '\0';
    return buf;
}
...
static ifnet_t *fetch_ifnet(char *name)
{
    ...
    if (stack == NULL) {
        printf("The stack '%s' doesn't exist\n", host);
        return NULL;
    }
    ifnet_t *ifnet = net_interface(stack, NET_AF_ETH, idx);
    if (ifnet == NULL) {
        printf("The device '%s:%d' doesn't exist on stack %s\n", prot, idx, host);
        return NULL;
    }
    return ifnet;
}
...
int do_ping(void *cfg, size_t *params)
{
    ...
    if (qry.success)
        printf("Ping request completed with success\n");
    else if (qry.received)
        printf("Ping request completed but wrong payload\n");
    else {
